...
base_dir = ""
user_embedding = torch.load(f'{base_dir}/model_weight/movielen/gcn_user_embedding.pkl')

user_embedding = nn.Embedding.from_pretrained(user_embedding)

item_embedding = torch.load(f'{base_dir}/model_weight/movielen/gcn_item_embedding.pkl')
...
def main():
    ...
    user_sequences_file = f'{base_dir}/data/movielen_for_train/user_sequences_id.json'
    ...
    train_title_file = f'{base_dir}/data/movielen_for_train/all_title_train.jsonl'
    train_cf_data_file = f'{base_dir}/data/movielen_for_train/train.jsonl'
   ...
   # title_dataloader4, cf_dataloader4 = get_dataloader(title_data_file=train_title_file, cf_data_file=train_cf_data_file, batch_size=8, f_filter=lambda x :x["task_type"]=="exp")
    ...
    optim1 = {1: ctr_optim1, 2: seq_optim1, 3: rating_optim1}
    lr_scheduler1 = {1: ctr_lr_scheduler1, 2: seq_lr_scheduler1, 3: rating_lr_scheduler1}
    optim2 = {1: ctr_optim2, 2: seq_optim2, 3: rating_optim2}
    lr_scheduler2 = {1: ctr_lr_scheduler2, 2: seq_lr_scheduler2, 3: rating_lr_scheduler2} 

    for epoch in range(EPOCHES):
    
        for j , (batch1, batch2, batch3, batch4, batch5, batch6) in tqdm(enumerate(zip(title_dataloader1, cf_dataloader1, title_dataloader2,cf_dataloader2,
                                                       title_dataloader3, cf_dataloader3)), total=len(cf_dataloader1), desc="train multi lora task"):
            net.train()
            loss_dict = {}
            batch_dict = {1:[batch1, batch2], 2: [batch3, batch4], 3: [batch5, batch6]}
        ...
        net.model.save_pretrained(save_directory=f"../model_weight/movie_lightgcn/epoch_{epoch}")

        print(f"------------------------------ {epoch+1} has finished------------------------------")
        torch.save(net.user_mapping, "../model_weight/movie_user_mapping_layer_gcn.pkl")
        torch.save(net.item_mapping, "../model_weight/movie_item_mapping_layer_gcn.pkl")
