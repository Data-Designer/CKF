## ! generate all title prompts without CF signal
...
def data_process(user_sequeneces_title:str, user_sequences_id:str, user_sequences_rating:str):
    ...
    id_2_title = load_json("../data/movie_len/id_2_title.json")
    all_item_id = set()
    ...
    return id_2_title, all_item_id, user_title_sequences, user_id_sequences, user_sequences_rating

def generate_data(id_2_title, all_item_id, user_title_sequences,user_id_sequences, user_sequences_rating):
    ...
    exp_train_data = []
    exp_test_data = []
    users_recode= load_json('../data/movie_len/cold_users.json')
    ...
    for user in tqdm(user_id_sequences,desc="generate train data"):
        ...
        for i in range(4):
            ...
            history_list , candidate_item_id = user_id_sequences[user][1:][start_pos:end_pos][:-1], user_id_sequences[user][1:][start_pos:end_pos][-1]   
            ...
            if i >= 2:
                ...
                start_pos = i + 1
                end_pos = i + 12
                history_list_sequences = user_id_sequences[user][1:][start_pos:end_pos]  ### 确保历史长度不超过10
            ...
            if i >= 3:
                ...
                history_list_title = user_title_sequences[user][start_pos:end_pos][:15][:-1]
                history_list_rating = user_sequences_rating[user][start_pos:end_pos][:15][:-1]
                ...
                tmp_rating["history_items_id"] = user_id_sequences[user][1:][start_pos:end_pos][:15][:-1]
                tmp_rating["candidate_item_title"] = id_2_title[str(target_item_id)]
                tmp_rating["output"] = user_sequences_rating[user][start_pos:end_pos][:15][-1]
                rating_train_data.append(tmp_rating)
          


    for user in tqdm(user_id_sequences,desc="generate test data"): 
        if len(user_id_sequences[user]) == 1: continue
        test_tmp_ctr = {}
        user_id = user_id_sequences[user][0]
        test_tmp_ctr["user_id"] = user_id
        test_history_length = random.randint(10,15)
        ...
        if random.random() < 0.5:
            ...
            test_tmp_ctr["output"] = "Yes."
        else:
            candidate_item_id = random.sample(all_item_id,1)[0]
            test_tmp_ctr["output"] = "No."
        test_tmp_ctr["candidate_item"] = [candidate_item_id] + [0] * 9
        test_tmp_ctr["candidate_item_title"] = id_2_title[str(candidate_item_id)]
    ...
    return ctr_train_data, ctr_test_data, sequential_train_data, sequential_test_data, rating_train_data, rating_test_data, cold_users
                        
def main():   
    id_2_title, all_item_id, user_title_sequences,user_id_sequences, user_rating_sequences = data_process('../data/movie_len/user_sequences_title.json', '../data/movie_len/user_sequences_id.json', '../data/movie_len/user_rating.json')
    ctr_train_data, ctr_test_data ,sequential_train_data, sequential_test_data, rating_train_data, rating_test_data, cold_users = generate_data(id_2_title, all_item_id, user_title_sequences, user_id_sequences, user_rating_sequences)
    ctr_train = []
    ctr_test = []
    sequential_train = []
    sequential_test = []
    ...
    for datas in tqdm([ctr_test, sequential_test, rating_test]):
    ...
    train_data = ctr_train + sequential_train + rating_train
    random.shuffle(train_data)

    # test_data = ctr_test + sequential_test + rating_test + exp_test
    with jsonlines.open('../data/movielen_for_train/all_title_train.jsonl', 'w') as writer:
